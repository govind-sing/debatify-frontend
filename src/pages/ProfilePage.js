...
const ProfilePage = () => {
  ...
  // ✅ FIX 1: Read the initial tab from the URL once at initialization.
  // Do NOT put this in a useEffect — reading it here avoids needing an effect at all
  // for the initial value, and prevents any effect from firing on first render.
  ...
  // ✅ FIX 2: Memoize the token with useRef so it never changes reference between renders.
  // getToken() returning a string on every render was causing all useCallback hooks
  // to see a "new" token and recreate themselves, which recreated fetchAllData,
  // which re-triggered the fetch useEffect — repeated Lambda calls on every render.
  const tokenRef = useRef(getToken());
  const token = tokenRef.current;

  // ✅ FIX 3: Sync URL → state only (never call navigate here).
  // The original effect called navigate() while also watching location.search.
  // navigate() changes location.search → re-triggers this effect → infinite loop.
  // Now this effect ONLY reads from the URL and updates state if needed.
  useEffect(() => {
    const section = new URLSearchParams(location.search).get("section");
  ...
  // ✅ FIX 4: navigate() is ONLY called from explicit user interaction (tab click),
  // never from inside a useEffect. This is the safe, correct pattern.
  ...
  // ✅ This is correct: re-fetch only when the username in the URL changes (navigating
  // to a different profile). fetchAllData is stable because token is now stable via useRef.
  useEffect(() => {
    fetchAllData();
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        <div className="flex justify-center space-x-10 border-b border-gray-200 pb-4 bg-white rounded-t-xl shadow-md">
          {["debate", "discussion", "blogs"].map((tab) => (
            <button
              key={tab}
              onClick={() => handleTabChange(tab)}  // ✅ uses handleTabChange, not setActiveTab directly
